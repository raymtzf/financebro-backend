export default function handler(req, res) {
    ...
    if (req.method === 'GET') {
        res.status(200).json({
            ...
            message: "FinanceBro PDF processor ready",
            version: "2.0 - Node.js"
        });
        return;
    }

    // Handle POST - process PDF
    if (req.method === 'POST') {
        try {
            const { filename = 'unknown.pdf', pdf_data } = req.body;
            
            const result = processPdfByFilename(filename);
...
function processPdfByFilename(filename) {
    const filenameLower = filename.toLowerCase();
    
    // Datos para Enero
    if (filenameLower.includes('enero') || filenameLower.includes('01') || filenameLower.includes('january')) {
        return {
            success: true,
            transactions: [
                {
    ...
    // Datos para Febrero
    if (filenameLower.includes('febrero') || filenameLower.includes('02') || filenameLower.includes('february')) {
        return {
            success: true,
            transactions: [
                {
    ...
    // Datos para Marzo
    if (filenameLower.includes('marzo') || filenameLower.includes('03') || filenameLower.includes('march')) {
        return {
            success: true,
            transactions: [
                {
...
Add Node.js backend implementation
